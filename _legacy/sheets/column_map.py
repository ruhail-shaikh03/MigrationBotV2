...
def resolve_column(user_term: str, column_map: dict | None = None) -> str | None:
    """
    ...
    Uses the session-level LLM-generated map (F11) if available,
    falling back to the static COLUMN_ALIASES.
    """
    try:
        import streamlit as st
        active_map = column_map or st.session_state.get('column_map', COLUMN_ALIASES)
    except Exception:
        active_map = column_map or COLUMN_ALIASES

    term = user_term.lower().strip()
    for canonical, aliases in active_map.items():
        if term == canonical.lower().strip() or term in aliases:
            return canonical
...
def get_column_map_json(column_map: dict | None = None) -> str:
    """
    Return the active column map as indented JSON for the system prompt.
